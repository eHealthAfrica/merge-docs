...
function table(diffs) {
  var renderer = new Table({ head: headers(diffs.length)
                           , colWidths: colWidths(diffs.length)
                           })
...
function colWidths(count) {
  var valueCols = _.range(count).map(function () { return 20 })
  return [30, 30].concat(valueCols)
}
...
function rhs(columns) {
  var value = _.chain(columns).pluck('[0].rhs').compact().first().value()
  return humanize(value)
}

function lhs(columns, rhs) {
  var values = columns.map(function (c) { return (c.length) ? c[0].lhs : rhs })
